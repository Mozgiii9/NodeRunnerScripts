...
install_node() {
    ...
    echo -e "${WHITE}[${CYAN}5/5${WHITE}] ${GREEN}➜ ${WHITE}⚙️ Установка дополнительных зависимостей...${NC}"
    cd $HOME/rl-swarm/modal-login
    npm install viem@2.22.6
    cd
    success_message "Дополнительные зависимости установлены"
    
    echo -e "\n${PURPLE}════════════════════════════════════════════════${NC}"
    echo -e "${GREEN}✨ Первый этап установки завершен успешно!${NC}"
    echo -e "${YELLOW}📝 Следуйте дальнейшим инструкциям в текстовом гайде.${NC}"
    echo -e "${PURPLE}════════════════════════════════════════════════${NC}\n"
}
...
remove_node() {
    ...
    if [ -n "$SESSION_IDS" ]; then
        ...
        success_message "Сессии успешно завершены"
    else
        warning_message "Сессии screen для ноды Gensyn не найдены, продолжаем удаление"
    fi
