...
print_menu() {
    echo -e "\n${BOLD}${WHITE}╔════════════════════════════════════════╗${NC}"
    echo -e "${BOLD}${WHITE}║      🌐 GAIA DOMAINS MANAGER v1.0       ║${NC}"
...
case $choice in
    ...
    2)
        ...
        echo -e "${WHITE}[${CYAN}1/7${WHITE}] ${GREEN}➜ ${WHITE}📝 Создание конфигурации...${NC}"
        mkdir -p ~/gaia-bot
        cd ~/gaia-bot
        ...
        echo -e "${WHITE}[${CYAN}2/7${WHITE}] ${GREEN}➜ ${WHITE}📥 Загрузка скрипта бота...${NC}"
        curl -L https://raw.githubusercontent.com/Mozgiii9/NodeRunnerScripts/main/gaia_bot_dom.py -o gaia_bot_dom.py

        echo -e "${WHITE}[${CYAN}3/7${WHITE}] ${GREEN}➜ ${WHITE}🌐 Настройка домена...${NC}"
        echo -e "${YELLOW}Введите название вашего домена без gaia.domains${NC}"
        echo -e "${CYAN}(если ваш домен forto.gaia.domains, то введите только forto)${NC}"
        read -p "➜ " NODE_ID
        
        sed -i "s|\$NODE_ID|$NODE_ID|g" gaia_bot_dom.py

        echo -e "${WHITE}[${CYAN}4/7${WHITE}] ${GREEN}➜ ${WHITE}🔑 Настройка API...${NC}"
        echo -e "${YELLOW}Введите ваш API ключ:${NC}"
        read -p "➜ " GAIA_API_KEY
        
        sed -i "s|\$GAIA_API_KEY|$GAIA_API_KEY|g" gaia_bot_dom.py

        echo -e "${WHITE}[${CYAN}5/7${WHITE}] ${GREEN}➜ ${WHITE}⚡ Настройка производительности...${NC}"
        ...
        echo -e "${WHITE}[${CYAN}6/7${WHITE}] ${GREEN}➜ ${WHITE}⚙️  Настройка сервиса...${NC}"
...
WantedBy=multi-user.target" > /etc/systemd/system/gaia-bot.service
        ...
        echo -e "${WHITE}[${CYAN}7/7${WHITE}] ${GREEN}➜ ${WHITE}🚀 Запуск сервиса...${NC}"
