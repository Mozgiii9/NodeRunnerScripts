...
print_menu() {
    ...
    echo -e "${WHITE}[${CYAN}11${WHITE}] ${GREEN}➜ ${WHITE}🚪 Выход${NC}\n"
}
...
install_ritual() {
    ...
    # Проверяем существование screen сессии
    if screen -ls | grep -q "ritual"; then
        echo -e "${YELLOW}⚠️ Сессия ritual уже существует. Переподключаемся...${NC}"
        screen -r ritual
    else
        # Создаем новую screen сессию с правильными параметрами
        ...
        echo -e "${CYAN}Для просмотра списка сессий используйте: screen -ls${NC}"
...
while true; do
    ...
    print_menu
    echo -e "${BOLD}${BLUE}📝 Введите номер действия [1-11]:${NC} "
    read -p "➜ " choice
    ...
    case $choice in
        ...
        11)
            ...
            ;;
        *)
            echo -e "\n${BOLD}${RED}❌ Ошибка: Неверный выбор! Пожалуйста, введите номер от 1 до 11.${NC}\n"
            ;;
    esac
